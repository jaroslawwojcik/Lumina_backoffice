...
const assetKindLabels: Record<string, string> = { image: 'Obraz', video: 'Wideo', audio: 'Audio', document: 'Dokument', subtitle: 'Napisy', transcript: 'Transkrypcja' }
...
export function MediaLibraryPage() {
  ...
  return <Box sx={{ maxWidth: 1440, p: { xs: 2, sm: 4 } }}>
    ...
    <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', mb: 3, p: 2.5 }}><FormControl size="small" sx={{ minWidth: 150 }}><InputLabel>Na stronie</InputLabel><Select label="Na stronie" onChange={(event) => { setPageSize(Number(event.target.value) as 25 | 50 | 100); setCursor(undefined); setCursorStack([]) }} value={pageSize}>{pageSizes.map((size) => <MenuItem key={size} value={size}>{size}</MenuItem>)}</Select></FormControl></Paper>
