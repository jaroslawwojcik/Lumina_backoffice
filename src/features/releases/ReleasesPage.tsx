...
function ReleaseListPage() {
  ...
  return <Box sx={{ maxWidth: 1440, p: { xs: 2, sm: 4 } }}>
    ...
    <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', mb: 3, p: 2.5 }}><FormControl size="small" sx={{ minWidth: 160 }}><InputLabel>Na stronie</InputLabel><Select label="Na stronie" onChange={(event) => changePageSize(event.target.value)} value={String(pageSize)}>{pageSizes.map((size) => <MenuItem key={size} value={String(size)}>{size}</MenuItem>)}</Select></FormControl></Paper>
