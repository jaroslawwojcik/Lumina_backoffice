...
import { Alert, Box, Button, Checkbox, Chip, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, InputLabel, MenuItem, Paper, Select, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography } from '@mui/material'
...
export function ReleaseComposerPage() {
  ...
  return <Box sx={{ maxWidth: 1200, p: { xs: 2, sm: 4 } }}>
    ...
    <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', mt: 3, p: { xs: 2, sm: 3 } }}>
    ...
    <Stack spacing={3} sx={{ mt: 3 }}>
      ...
      {release && <ReleaseResult release={release} canActivate={canActivate} isActivating={activateMutation.isPending} activationError={activateMutation.isError} publishedAt={activateMutation.data?.publishedAt} resourceCount={selectedResources.length} locale={locale} onActivate={() => activateMutation.mutate(release.releaseId)} />}
    </Stack>
  </Box>
}

function ReleaseResult({ release, canActivate, isActivating, activationError, publishedAt, resourceCount, locale, onActivate }: { release: BuildReleaseResponse; canActivate: boolean; isActivating: boolean; activationError: boolean; publishedAt?: string; resourceCount: number; locale: string; onActivate: () => void }) {
  const [confirmOpen, setConfirmOpen] = useState(false)
  const confirmActivation = () => { setConfirmOpen(false); onActivate() }
  return <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', p: { xs: 2, sm: 3 } }}><Typography component="h2" variant="h5">Wydanie gotowe do aktywacji</Typography><Stack spacing={1} sx={{ mt: 2 }}><Typography><strong>Zasoby:</strong> {resourceCount}</Typography><Typography><strong>Język:</strong> {locale === 'pl' ? 'Polski' : 'Angielski'}</Typography><Typography><strong>ID wydania:</strong> {release.releaseId}</Typography><Typography><strong>ETag katalogu:</strong> {release.catalogEtag}</Typography><Typography sx={{ overflowWrap: 'anywhere' }}><strong>Klucz storage:</strong> {release.catalogStorageKey}</Typography></Stack>{publishedAt && <Alert severity="success" sx={{ mt: 2 }}>Wydanie aktywowano: {formatDate(publishedAt)}.</Alert>}{activationError && <Alert severity="error" sx={{ mt: 2 }}>Nie udało się aktywować wydania. Stan operacji może wymagać sprawdzenia po stronie serwera.</Alert>}{canActivate && !publishedAt && <Button disabled={isActivating} onClick={() => setConfirmOpen(true)} startIcon={<PublishOutlined />} sx={{ mt: 3 }} variant="contained">{isActivating ? 'Aktywowanie…' : 'Sprawdź i aktywuj'}</Button>}{!canActivate && <Alert severity="info" sx={{ mt: 2 }}>Do aktywacji wydania potrzebujesz uprawnienia `release.activate`.</Alert>}<Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}><DialogTitle>Aktywować to wydanie?</DialogTitle><DialogContent><Typography>Aktywny katalog zostanie zastąpiony wydaniem obejmującym {resourceCount} zasobów w języku {locale === 'pl' ? 'polskim' : 'angielskim'}. Użytkownicy zaczną widzieć tę wersję po publikacji.</Typography></DialogContent><DialogActions><Button onClick={() => setConfirmOpen(false)}>Wróć do podglądu</Button><Button onClick={confirmActivation} startIcon={<PublishOutlined />} variant="contained">Aktywuj wydanie</Button></DialogActions></Dialog></Paper>
}
