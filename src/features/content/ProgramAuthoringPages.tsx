...
export function CreateProgramPage() {
  ...
  return <Box sx={{ maxWidth: 900, p: { xs: 2, sm: 4 } }}>
    ...
    <Paper component="form" elevation={0} noValidate onSubmit={form.handleSubmit(submit)} sx={{ border: '1px solid', borderColor: 'divider', mt: 3, p: { xs: 2, sm: 3 } }}><Stack spacing={2.5}>
...
export function ProgramEditorPage() {
  ...
  if (draftQuery.isPending) return <Box sx={{ p: { xs: 2, md: 4 } }}><Typography aria-live="polite">Ładowanie programu...</Typography></Box>
  if (draftQuery.isError) { const absent = draftQuery.error instanceof ApiError && draftQuery.error.status === 404; return <Box sx={{ maxWidth: 900, p: { xs: 2, md: 4 } }}><Alert action={!absent && <Button color="inherit" onClick={() => void draftQuery.refetch()} startIcon={<ReplayOutlined />}>Spróbuj ponownie</Button>} severity="error">{absent ? 'Nie znaleziono tego programu lub tłumaczenia.' : 'Nie udało się pobrać programu.'}</Alert></Box> }
