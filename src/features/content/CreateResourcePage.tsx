...
import { ExistingMediaSelect } from '../media/ExistingMediaSelect'
  ...
  return <Box sx={{ maxWidth: 900, p: { xs: 2, sm: 4 } }}>
    ...
    <Paper component="form" elevation={0} noValidate onSubmit={form.handleSubmit(submit)} sx={{ border: '1px solid', borderColor: 'divider', mt: 3, p: { xs: 2, sm: 3 } }}>
      <Stack spacing={2.5}>
        ...
        {resourceType === 'session' && hasAnyPermission(permissions, ['media.read']) && <ExistingMediaSelect kind={mediaKind} label={mediaKind === 'video' ? 'Wideo z biblioteki' : 'Audio z biblioteki'} value={mediaKind === 'video' ? primaryAssetId : audioAssetId} disabled={mutation.isPending || uploadBusy} onChange={(asset) => {
          if (mediaKind === 'video') setPrimaryAssetId(asset?.id)
          else setAudioAssetId(asset?.id)
          if (asset?.durationSeconds) form.setValue('durationSeconds', String(asset.durationSeconds), { shouldValidate: true })
        }} />}
        {hasAnyPermission(permissions, ['media.read']) && <ExistingMediaSelect kind="image" label="Obrazek z biblioteki" value={thumbnailAssetId} disabled={mutation.isPending || uploadBusy} onChange={(asset) => setThumbnailAssetId(asset?.id)} />}
