...
export function ResourceDetailPage({ expectedType }: { expectedType: 'session' | 'material' }) {
  ...
  if (detailQuery.isPending) return <Box sx={{ p: { xs: 2.25, md: 3.25 } }}><Typography aria-live="polite">Ładowanie zasobu...</Typography></Box>
  if (detailQuery.isError) {
    const isNotFound = detailQuery.error instanceof ApiError && detailQuery.error.status === 404
    return <Box sx={{ maxWidth: 900, p: { xs: 2.25, md: 3.25 } }}><Alert action={!isNotFound && <Button color="inherit" onClick={() => void detailQuery.refetch()} startIcon={<ReplayOutlined />}>Spróbuj ponownie</Button>} severity="error">{isNotFound ? 'Nie znaleziono tego zasobu.' : 'Nie udało się pobrać zasobu.'}</Alert></Box>
  }
  const detail = detailQuery.data
  if (detail.resourceType !== expectedType) return <Box sx={{ maxWidth: 900, p: { xs: 2.25, md: 3.25 } }}><Alert severity="error">Nie znaleziono tego zasobu.</Alert></Box>
  ...
  return <Box sx={{ maxWidth: 1100, p: { xs: 2.25, md: 3.25 } }}>
    ...
    <Paper elevation={0} sx={{ border: '1px solid #e4e1e9', mb: 3, p: 2.5 }}><Stack direction="row" spacing={3} sx={{ flexWrap: 'wrap' }}><Typography>Typ: {detail.resourceType}</Typography><Typography>Wersja: {detail.version}</Typography><Typography>Utworzono: {formatDate(detail.createdAt)}</Typography><Typography>Aktualizacja: {formatDate(detail.updatedAt)}</Typography><FormControl size="small" sx={{ minWidth: 160 }}><InputLabel>Język</InputLabel><Select label="Język" value={selectedLocale.toLowerCase()} onChange={(event) => { const next = new URLSearchParams(searchParams); next.set('locale', event.target.value); setSearchParams(next) }}><MenuItem value="pl">Polski</MenuItem><MenuItem value="en">Angielski</MenuItem><MenuItem value="pl-pl">Polski (Polska)</MenuItem><MenuItem value="en-us">Angielski (USA)</MenuItem></Select></FormControl></Stack></Paper>
    <Stack spacing={3}>{canEdit && detail.session?.mediaKind === 'audio' && hasAnyPermission(permissions, ['media.upload']) && <SessionAudioUpload key={`audio-${resourceId}`} resourceId={resourceId} expectedRevisionId={detail.currentRevision?.revisionId} />}{canEdit && hasAnyPermission(permissions, ['media.upload']) && <ThumbnailImageUpload key={`image-${resourceId}`} resourceId={resourceId} expectedRevisionId={detail.currentRevision?.revisionId} currentUrl={assetsQuery.data?.find((item) => item.role === 'thumbnail' && item.locale === null && item.position === 0)?.asset.publicUrl} />}{canEdit && detail.session?.mediaKind === 'video' && hasAnyPermission(permissions, ['media.upload']) && <SessionVideoUpload key={resourceId} resourceId={resourceId} expectedRevisionId={detail.currentRevision?.revisionId} />}{canEdit && <ResourceRevisionForm key={`${resourceId}-${selectedLocale}`} detail={detail} />}<Paper elevation={0} sx={{ border: '1px solid #e4e1e9', p: 2.5 }}><Typography component="h2" variant="h5" sx={{ mb: 2 }}>Aktualna rewizja</Typography><TextField aria-label="Aktualny zrzut JSON" fullWidth multiline minRows={12} value={displayedSnapshot} slotProps={{ input: { readOnly: true } }} /></Paper>
