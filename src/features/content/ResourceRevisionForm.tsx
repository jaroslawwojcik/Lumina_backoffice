...
export function ResourceRevisionForm({ detail }: { detail: ResourceDetail }) {
  // Stable resource fields (media kind, duration, featured, intensity) are not
  // revision data. Keeping them out of the JSON avoids suggesting that saving
  // a revision updates the typed content tables.
  const initial = detail.currentRevision?.snapshot ?? {}
  ...
  const mutation = useMutation({ mutationFn: (snapshot: Record<string, unknown>) => createResourceRevision(detail, snapshot, note.trim() || undefined), retry: false, onSuccess: async () => {
    setNote('')
    await Promise.all([client.invalidateQueries({ queryKey: ['resource', detail.resourceId] }), client.invalidateQueries({ queryKey: ['program-draft', detail.resourceId] }), client.invalidateQueries({ queryKey: ['resource-revisions', detail.resourceId] }), client.invalidateQueries({ queryKey: ['resources'] })])
  } })
  const save = () => {
  ...
  return <Paper component="form" noValidate onSubmit={(event) => { event.preventDefault(); save() }} variant="outlined" sx={{ p: 2.5 }}>
    <Stack spacing={2}>
      ...
      <SnapshotEditor label="Zrzut JSON nowej rewizji" value={text} onChange={(value) => { setText(value); setErrors([]); mutation.reset() }} disabled={mutation.isPending} suggested={detail.resourceType === 'session' ? sessionPublicMetadataSuggestion : {}} />
